...
CAPTION_FILE = './inputs/repaint_mirai.txt'
...
SRC_AUDIO = 'output/20260419_022435/02.mp3'

# 再生成する区間（秒）
#REPAINTING_START = 140.0    # 再生成開始時刻（秒）
#REPAINTING_END   = 166.0   # 再生成終了時刻（秒）、-1 でファイル末尾まで
REPAINTING_START = 119.4    # 再生成開始時刻（秒）
REPAINTING_END   = 125.0   # 再生成終了時刻（秒）、-1 でファイル末尾まで

# 元音声構造の保持強度（repaintタスクの場合は必ず 1.0 に設定）
AUDIO_COVER_STRENGTH = 1.0

# Repaint モード
#   'conservative' : 全ステップで injection（最も周囲に馴染む、短区間修正に推奨）
#   'balanced'     : 前半50%のみ injection（デフォルト）
#   'aggressive'   : injection なし（完全新規生成、waveform splice もなし）
REPAINT_MODE     = 'conservative'
# Repaint 強度（balanced モードのみ有効。0.0=保守的、1.0=積極的。conservative/aggressive では無視）
...
BPM             = 188 # 曲のテンポ（BPM）。None で自動推定
KEYSCALE        = "E minor"  # 曲のキー（例: 'C', 'G#'）。"" で自動推定
TIMESIGNATURE    = "4"  # 曲の拍子（例: '4/4'）。"" で自動推定
DURATION        =  275   # 生成時間（秒）。-1 で自動
